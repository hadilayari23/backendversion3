...
async CreateUser(userId: string, createUserDto: CreateUserDto): Promise<Users> {
    ...
    try {
        ...
        if (user.roles === Role.SuperAdmin) {
            newUserRoles = Role.Admin;
        } else if (user.roles === Role.Admin) {
            newUserRoles = Role.Viewer;
        } else {
            throw new BadRequestException('User not authorized to create new users');
        }
        ...
        // Copier les appareils de l'admin au nouvel utilisateur
        if (newUserRoles === Role.Viewer) {
            const adminUser = await this.usersModel.findOne({ roles: Role.Admin });
            if (adminUser) {
                newUser.devices = adminUser.devices;
            }
        }
        ...
        } else if (newUserRoles === Role.Viewer) {
            subject = 'Welcome to AquaOptim';
            message = `Welcome to AquaOptim. Your email: ${createUserDto.email}, your password: ${randomPassword}`;
        } else {
